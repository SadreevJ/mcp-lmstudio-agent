...
- выбор директории из `workspace` и подготовка проекта к новому чату (`prepare-chat`).

Меню показывает короткие операторские сигналы:
- `OK` — шаг выполнен;
- `Требуется действие` — нужен ваш следующий шаг (или исправление ошибки).
...
2. Выберите пункт `3` и введите `test2` (из списка папок `workspace`)
